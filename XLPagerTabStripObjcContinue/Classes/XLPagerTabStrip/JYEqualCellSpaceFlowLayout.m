    ...
    for (NSUInteger index = 0; index < layoutAttributes.count ; index++) {
        ...
        if (currentY != previousY && currentY != nextY){
            if ([currentAttr.representedElementKind isEqualToString:UICollectionElementKindSectionHeader]) {
                ...
                if (_sumCellWidth < self.collectionView.bounds.size.width || self.scrollDirection == UICollectionViewScrollDirectionVertical) {
        ...
        //如果下一个cell在本行，这开始调整Frame位置
            ...
            if (self.collectionView.contentSize.width > self.collectionView.frame.size.width && self.scrollDirection == UICollectionViewScrollDirectionHorizontal) {
